...
def test_log_review_and_digest_journey(client):
    ...
    r1 = client.post("/api/v1/attempts", json=dict(
    ...
    client.post("/api/v1/attempts", json=dict(
    ...
    hard = client.get("/api/v1/problems?min_rating=4&solved_self=false").json()
    ...
    assert client.get(f"/api/v1/problems/{pid}/similar").status_code == 200

    # 5. weekly digest on demand
    digest_body = client.post("/api/v1/stats/digest/send-now").json()
    assert digest_body["stats"]["total"] == 2
    assert isinstance(digest_body["note"], str) and digest_body["note"]

    # 6. edit then delete
    assert client.patch(f"/api/v1/problems/{pid}", json={"rating": 4}).status_code == 200
    assert client.delete(f"/api/v1/problems/{pid}").status_code == 204
    assert len(client.get("/api/v1/problems").json()) == 1
